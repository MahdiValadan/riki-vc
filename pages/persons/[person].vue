<template>
    <div class="flex flex-col min-h-screen items-center py-20">
        ...
        <div class="flex flex-row flex-wrap bg-white w-9/12 min-h-[500px] rounded-md shadow-md">
            <!-- Left Side -->
            <div id="image" class="w-full md:w-6/12 py-8 px-12">
                <img :src="person.image" alt="project image">
            </div>
            <!-- Right Side -->
            <div class="flex flex-col gap-6 w-full md:w-6/12 p-8">
                <h2 id="name" class="text-3xl text-black font-bold">
                    {{ person.name }}
                </h2>
                <h2 id="role" class="text-[#FF8F52]">
                    {{ person.role }}
                </h2>
                <h2 class="text-black">
                    <div id="managering_projects" class="flex flex-col gap-2">
                        <span class="font-bold">Work in these projects</span>
                        <div id="project_buttons" class="flex flex-center gap-3">
                            <OrangeButton v-for="(projectName, index) in projectNames" :key="index"
                                :buttonText="projectName" :link="`/projects/${index}`"></OrangeButton>
                        </div>
                    </div>
                </h2>
                <h2 id="cv" class="text-black">
                    {{ person.cv }}
                </h2>
            </div>
        </div>
